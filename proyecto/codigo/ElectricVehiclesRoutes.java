...
public class ElectricVehiclesRoutes {
    ...
    public ElectricVehiclesRoutes(String filename) {    
        ...
        try {
            ...
            for (int i = 0; i < m; i++) {   
                for (int j = 0; j < m; j++) {
                    double time = (Math.sqrt(Math.pow(coordinates.get(i).first - coordinates.get(j).first,2)+ 
                                Math.pow(coordinates.get(i).second - coordinates.get(j).second, 2)))/speed;
                    map.addArc(i, j, time);
                }
            }                  
    ...
    public void solution(boolean test) {
        routeConstruction routeResult = new routeConstruction();
        routeResult.savingsCalculation(this.map, this.map.size());
        routeResult.calculadorRuta(this.map);
        if (test) {
